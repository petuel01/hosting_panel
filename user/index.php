...
<body class="container mt-5">
    ...
    <!-- Buttons for actions -->
    <div class="mb-3">
        <button onclick="loadFiles()" class="btn btn-primary">Load Files</button>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <button type="button" class="btn btn-primary mt-3" onclick="saveFile()">Save</button>
                </div>
            </div>
        </div>
    </div>
            ...
            fetch(`list_files.php?username=${username}&path=${encodeURIComponent(currentPath)}`)
                ...
                .then(items => {
                    ...
                    items.forEach(item => {
                        ...
                        row.innerHTML = `
                            <td>
                                ${item.type === 'folder' ? `<a href="#" onclick="navigateTo('${item.name}')">${item.name}</a>` : item.name}
                            </td>
                            <td>${item.type}</td>
                            <td>${item.size}</td>
                            <td>${item.modified}</td>
